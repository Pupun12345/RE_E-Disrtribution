...
export default function DistributionPage() {
  ...
  const fetchDistributions = async () => {
    try {
      const res = await api.get("/api/distribution");
  ...
  const handleAddDistribution = async () => {
    ...
    try {
      const res = await api.post("/api/distribution", {
  ...
  const updateDistribution = async () => {
    ...
    try {
      const res = await api.put(
        `/api/distribution/${editRecord._id}`,
        editRecord
      );
      if (res.success) {
        setRecords((prev) =>
  ...
  const exportPDF = () => {
    const doc = new jsPDF();
    doc.text("PPE Distribution Report", 14, 15);
    autoTable(doc, {
      startY: 25,
  ...
  return (
    <div className={styles.container}>
      <h2 className={styles.title}>PPE Distribution</h2>
