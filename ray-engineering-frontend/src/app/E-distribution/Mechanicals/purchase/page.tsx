...
export default function MechanicalPurchasePage() {
  ...
  const fetchPurchases = async () => {
    try {
      const res = await api.get("/api/mechanical/purchases");
      const list = Array.isArray(res) ? res : res.purchases || [];
      setPurchases(list);
    } catch (err) {
      console.error("Error fetching mechanical purchases:", err);
      showMessage("⚠️ Failed to fetch mechanical purchase records");
    }
  };
  ...
  const handleSave = async () => {
    ...
    try {
      if (editingId) {
        await api.put(`/api/mechanical/purchases/${editingId}`, payload);
        showMessage("✅ Purchase updated successfully");
      } else {
        await api.post("/api/mechanical/purchases", payload);
  ...
  const generateInvoicePDF = () => {
    ...
    const addHeader = () => {
      ...
      doc.setFontSize(16);
      doc.text("MECHANICAL INVOICE", pageWidth / 2, 55, {
        align: "center",
      });
  ...
  return (
    <div className={styles.container}>
      <div className={styles.wrapper}>
        <h1 className={styles.title}>Mechanical Purchase Entry</h1>
        ...
        {activeTab === "report" && (
          <>
            <h2 className={styles.title}>Mechanical Purchase Report</h2>
